...
export class NotificationsPage implements OnInit {
  ...
  async loadNotifications() {
    this.isLoading = true;
    try {
      const response = await lastValueFrom(this.authService.getNotifications());
  ...
  async toggleReadStatus(notification: any, read: boolean) {
    try {
      await lastValueFrom(this.authService.updateNotificationReadStatus(notification._id, read));
